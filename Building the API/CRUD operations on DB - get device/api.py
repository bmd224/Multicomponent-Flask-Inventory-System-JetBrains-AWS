...
def device(identifier):
    if request.method == 'GET':
        # use DAL instead of the in-memory devices dictionary
        device_data = dal.get_device(identifier)

        # return 404 if not found
        if device_data is None:
            return jsonify({'message': 'Device not found'}), 404

        # serialize the device using Marshmallow and return JSON
        return jsonify(device_schema.dump(device_data)), 200
